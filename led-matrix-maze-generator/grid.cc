...
void Grid::populateNeighbors() {
  // this lambda function can determine if a cell has a neighbor in a given
  // direction by checking the cell's location in the grid.
    ...
    switch (dir) {
      ...
      default:
        throw DirectionException(
            "A direction that is not defined was attempted for use.");
    }
  };
  ...
  auto addNeighborsToCell = [&](Cell *cell, std::size_t row,
                                std::size_t column) {
...
Grid::Grid(rgb_matrix::Canvas *canvas, unsigned int height,
           unsigned int width) {
  this->canvas = canvas;
  this->height = height;
  this->width = width;
...
Grid::~Grid() { this->canvas->Fill(0, 0, 0); }
