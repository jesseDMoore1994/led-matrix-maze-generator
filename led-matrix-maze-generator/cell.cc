...
// position
Cell::Cell(unsigned int matrix_x_position, unsigned int matrix_y_position,
...
unsigned int Cell::get_grid_row() { return this->grid_row; }

// returns the column position of the cell in the grid
unsigned int Cell::get_grid_column() { return this->grid_column; }
...
// This function takes a reference to a cell and it to the neighbors array at
// the specified direction.
...
// This function gets the reference to a neighboring cell if it exits and
// throws an error if no neighbor exits in that direction.
Cell Cell::getNeighbor(direction dir) {
  assertNeighborExists(dir);
  return *(this->neighbors[dir].first);
}

// This function "turns on" a connection between two cells by setting the
// boolean for the for the neighboring cell to true and then signaling the
// other cell to do the same for the direction of this cell if it has not been
// done already.
...
// This function returns the state of the connection between the current cell
// and the neighbor at the specified direction.
...
// visitation status.
std::set<direction> Cell::getValidDirectionsWithVisitedStatus(
    bool visited_status) {
...
// In order to draw the cell, we make sure to set the cell core on, then we
// draw a connection to each of the neighboring cells that are connected.
void Cell::draw(rgb_matrix::Canvas *canvas) {
  setColor();
  canvas->SetPixel(this->matrix_x_position, this->matrix_y_position, this->red,
                   this->green, this->blue);
  if (neighbors[Left].second) {
    canvas->SetPixel(this->matrix_x_position, this->matrix_y_position - 1,
                     this->red, this->green, this->blue);
  }
  if (neighbors[Up].second) {
    canvas->SetPixel(this->matrix_x_position + 1, this->matrix_y_position,
                     this->red, this->green, this->blue);
  }
